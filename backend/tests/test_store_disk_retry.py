...
session in store._dirty (so the next flush retries it) instead of silently
dropping it, and a successful flush must leave _dirty clean."""
import pandas as pd
import pytest

from services import store, store_disk
...
def test_write_failure_requeues_session(disk_cache, monkeypatch, caplog):
    ...
    monkeypatch.setattr(store_disk, "_atomic_write_pickle", boom)
    with caplog.at_level("WARNING", logger="services.store_disk"):
        store_disk.flush_dirty_to_disk()
...
def test_unwritable_cache_dir_requeues_all(disk_cache, monkeypatch, caplog):
    ...
    monkeypatch.setattr(store_disk.os, "makedirs", no_dir)
    with caplog.at_level("WARNING", logger="services.store_disk"):
        store_disk.flush_dirty_to_disk()
...
def test_successful_flush_clears_dirty(disk_cache):
    ...
    df_path, meta_path = store_disk._cache_paths(sid)
    import os
