...
The optional snapshot-to-disk layer (so a container restart/redeploy doesn't
silently wipe a user's in-progress edits) lives in services/store_disk.py;
this module keeps the session state it reads and the _dirty set that drives
its flushes. The same TTL/MAX_SESSIONS eviction that prunes memory also
deletes the matching disk files, via _purge_locked.
"""
import logging
import os
import tempfile
...
# ── Disk autosave config (opt-in — OFF by default) ─────────────────────────
# The implementation lives in services/store_disk.py; only the config
# constants stay here, next to the session state that module reads. Privacy
# default: uploaded data lives ONLY in server RAM and is never written to
# disk (see store_disk.py's docstring for the full rationale and the
# unencrypted-data warning that applies when an operator opts in).
...
def _delete_disk_snapshot(session_id: str) -> None:
    """Forward to store_disk lazily — the import cannot sit at the top of this
    module because store_disk imports this module for the session state."""
    from services import store_disk
    store_disk.delete_disk_snapshot(session_id)
...
def load_persisted_sessions() -> None:
    """Rehydrate the store from disk snapshots on backend startup. Importing
    store_disk here (not at the top — see _delete_disk_snapshot) also starts
    its autosave thread when the disk cache is opted in."""
    from services import store_disk
    store_disk.load_persisted_sessions()
