...
function ModelsCombo() {
  const [sub, setSub] = usePersistedPanelState<"regression" | "survival" | "rcs" | "threshold" | "multimodel" | "ml" | "timeseries" | "validation">("combo_models", "sub", "regression");
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      <div className="flex gap-1 px-4 pt-2 pb-1 bg-tint border-b border-line flex-shrink-0">
        {([["regression", "Regression"], ["survival", "Survival Advanced"], ["rcs", "Restricted Cubic Spline"], ["threshold", "Threshold"], ["multimodel", "Progressive Adjustment"], ["ml", "Machine Learning", "dev"], ["timeseries", "Time Series"], ["validation", "Validation (internal / external)"]] as const).map(([id, label, badge]) => (
          <button key={id} onClick={() => setSub(id)}
