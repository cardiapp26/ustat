...
import { exportDataset, downloadSessionJson, downloadProjectFile, downloadAnalysisScript, type ExportFmt } from "./lib/exportDataset";
...
import SubgroupPanel from "./components/SubgroupPanel";
...
function ModelsCombo() {
  const [sub, setSub] = usePersistedPanelState<"regression" | "survival" | "rcs" | "threshold" | "multimodel" | "subgroup" | "ml" | "timeseries" | "validation">("combo_models", "sub", "regression");
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      <div className="flex gap-1 px-4 pt-2 pb-1 bg-tint border-b border-line flex-shrink-0">
        {([["regression", "Regression"], ["survival", "Survival Advanced"], ["rcs", "Restricted Cubic Spline"], ["threshold", "Threshold"], ["multimodel", "Progressive Adjustment"], ["subgroup", "Subgroups"], ["ml", "Machine Learning", "dev"], ["timeseries", "Time Series"], ["validation", "Validation (internal / external)"]] as const).map(([id, label, badge]) => (
          <button key={id} onClick={() => setSub(id)}
      ...
      <div className="flex-1 overflow-y-auto">
        {sub === "regression" ? <div className="p-4"><ModelsPanel /></div>
          ...
          : sub === "multimodel" ? <div className="p-4"><MultiModelPanel /></div>
          : sub === "subgroup" ? <div className="p-4"><SubgroupPanel /></div>
            ...
            <div className="relative" ref={headerSaveMenuRef}>
              ...
              {showHeaderSaveMenu && session && (
                <div className="absolute right-0 top-full mt-1 w-48 bg-white border border-gray-200 rounded-xl shadow-xl z-50 overflow-hidden">
                  ...
                  <button
                    onClick={() => { setShowHeaderSaveMenu(false); downloadAnalysisScript(session); }}
                    className="w-full flex items-center gap-2.5 px-3 py-2 text-left hover:bg-gray-50 transition-colors"
                  >
                    <div>
                      <p className="text-xs text-gray-700 font-medium">Replay script (.py)</p>
                      <p className="text-[10px] text-gray-400">Import + prep steps + analysis definitions</p>
                    </div>
                  </button>
                  <button
